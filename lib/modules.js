    ...
    if (buildCfg.build) {
        if (buildCfg.build.files) {
            if (!buildCfg.build.builddir) {
                throw 'There is a "build.files" property but not a "build.builddir" property in fr4mebuild.json in directory: ' + buildConfigPath;
            }
 ...
 */
function Module(name, modulePath, builddir, files) {
    this.name = name;
